...
// returning the pointer to the thing stored with the symbol or NULL if it could not be found
...
// lookup the entry in the symbol table. This returns the entire entry in the table.
...
// returns the beginning of the symbol table. Note that this is a scope separator
SymTabEntry *SymTab::firstSymTabEntry() {
    return table_;
}

// returns the first SymTabEntry ptr that has scope scopeName
...
// the depth of the scope stack with the first real scope (probably globals) numbered 1
int SymTab::depth() {
    return scopeDepth_;
}

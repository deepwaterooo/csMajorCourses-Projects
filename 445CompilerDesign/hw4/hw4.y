...
var_declaration : type_specifier var_decl_list ';' 
		  ...
		  yyerrok;
                ...
                | type_specifier error ';'            {$$=NULL; yyerrok;}
                ;
/*
scoped_var_declaration : scoped_type_specifier var_decl_list ';' 
			 ...
			 yyerrok;
		       ...
		       | scoped_type_specifier error ';'           {$$=NULL; yyerrok;}
                       ;
...
var_decl_list : var_decl_list ',' var_decl_initialize 
		...
		yyerrok;
	      ...
	      | error ',' var_decl_initialize {$$=NULL;  yyerrok;}   //modify           
...
var_decl_initialize : ID 
		      ...
		      $$->isArray = true;
                    ...
                    | ID ':' error     {$$=NULL;}   
		    | ID '[' NUMCONST ']' ':' error {$$=NULL;}   
         	    | ID '[' NUMCONST error      {$$=NULL;}   
	            | error '[' NUMCONST ']'        {$$=NULL; yyerrok;}
                    ;
*/
var_decl_initialize : var_decl_id {$$=$1;}
		    ...
		    | error ':' simple_expression  {$$=NULL; yyerrok;}
...
fun_declaration : type_specifier ID '(' params ')' statement 
		...
		| type_specifier error '(' params ')' statement   {$$=NULL; yyerrok;}
		| type_specifier ID '(' error                     {$$=NULL;}
		| ID '(' error ')' statement                      {$$=NULL; yyerrok;}
		| error ')' statement                             {$$=NULL; yyerrok;}
		;
/*
param_list : param_list ';' type_specifier param_id_list 
	   ...
	   | error ';' type_specifier param_id_list  {$$=NULL; yyerrok;}
	   | param_list ';' type_specifier error     {$$ = $1;}
...
//           | param_list ';' error {$$=NULL;}
           | error ';' param_type_list   {$$=$3; yyerrok;} //NULL
           ;
...
param_id_list : param_id_list ',' param_id 
		...
		yyerrok;
              ...
              | param_id { $$ = $1; }
              | error ',' param_id  {$$=$3; yyerrok;}        //$3
              | param_id_list ',' error  {$$=$1; yyerrok;}   //$1
              ;
param_id : ID 
         {
	   $$ = newDeclNode(ParamK);
...
ifmatched : IF '(' simple_expression ')' ifmatched ELSE ifmatched 
	  ...
	  | IF '(' error      {$$=NULL;}
	  | error ')' ifmatched ELSE ifmatched  {$$=NULL; yyerrok;}
...
ifunmatched : IF '(' simple_expression ')' ifmatched ELSE ifunmatched
	      ...
	      $$->child[1] = $5;
            ...
            | IF '(' simple_expression ')' error ELSE ifunmatched    {$$=NULL; yyerrok;}
       	    | IF '(' error           {$$=NULL;}
//	    | error ')' ifunmatched  {$$=NULL; yyerrok;}
	    | error ')' statement    {$$=NULL; yyerrok;}
	    ...
	    | FOREACH '(' error        {$$=NULL; }
//	    | FOREACH '(' error ')' IN simple_expression ')' ifunmatched     {$$=NULL; yyerrok;}
//          | FOREACH error mutable IN simple_expression ')' ifunmatched     {$$=NULL; yyerrok;}
//	    | FOREACH '(' error ')' ifunmatched     {$$=NULL; yyerrok;}
            ;
...
compound_stmt : '{' local_declarations statement_list '}' 
		...
		yyerrok;
	      ...
	      | error '}'                      {$$=NULL; yyerrok;}
	      | '{' error statement_list '}'   {$$=NULL; yyerrok;}
	      ;

...
class wassAlerts {
	...
	*/
	static function create($calendarid,$datestart,$timestart,$dateend,$timeend,$userid,$email,$message,$type) {
		...
		$insert = wassSQL::doQUERY('INSERT INTO ' . wassParms::DATABASE . '.wassAlerts (calendarid,datestart,timestart,dateend,timeend,userid,email,message,type) VALUES ('
	...
	*/
	static function issue($alertid) {
		...
		/* Read in the alert */
		$alert = wassSQL::doFetch(wassSQL::doQuery('SELECT * FROM ' . wassParms::DATABASE . '.wassAlerts WHERE alertid=' . wassSQL::sqlSafe($alertid) . ';'));
		...
		if ($alert) {
			...
			wassSQL::doQUERY('UPDATE ' . wassParms::DATABASE . '.wassAlerts SET sentcount=sentcount+1 WHERE alertid=' . $alertid . ' LIMIT 1;');
		}
	...
	*/
	static function issueall($datestart,$timestart,$dateend,$timeend,$message='') {
		...
		$alerts = wassSQL::doQuery('SELECT * FROM ' . wassParms::DATABASE . '.wassAlerts WHERE ((datestart>=' . wassSQL::sqlSafe($datestart) . ' AND timestart>=' . wassSQL::sqlSafe($timestart) . ') AND (dateend<=' . wassSQL::sqlSafe($dateend) . ' AND timeend<=' . wassSQL::sqlSafe($timeend) .'));');
		...
		while ($alert = wassSQL::doFetch($alerts)) {
			...
			/* And update the sent count */
			wassSQL::doQUERY('UPDATE ' . wassParms::DATABASE . '.wassAlerts SET sentcount=sentcount+1 WHERE alertid=' . $alert['alertid'] . ' LIMIT 1;');
		}

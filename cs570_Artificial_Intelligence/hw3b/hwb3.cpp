...
public:
  ...
  void BuildMoonLanderBurnRules()
  {
    ...
    height->AddLinguisticValue(new LinguisticValue("Low",0,0,25,40));
    height->AddLinguisticValue(new LinguisticValue("Average",30,45,70,80));
    height->AddLinguisticValue(new LinguisticValue("High",75,90,100,100));
    ...
    burn->AddLinguisticValue(new LinguisticValue("NoBurn",0,0,1,1.5));
    burn->AddLinguisticValue(new LinguisticValue("SlightBurn",1,1.5,2,2.5));
    burn->AddLinguisticValue(new LinguisticValue("HardBurn",2, 2.5, 3, 3));
  ...
  void BuildMoonLanderThrustRules()
  {
    ...
    position->AddLinguisticValue(new LinguisticValue("Left",-0.3,-0.3,-0.1,-0.05));
    position->AddLinguisticValue(new LinguisticValue("Middle",-0.1,-0.05,0.05,0.1));
    position->AddLinguisticValue(new LinguisticValue("Right",0.05,0.1,0.3,0.3));
    ...
    thrust->AddLinguisticValue(new LinguisticValue("NoThrust", 0, 0, 0.1, 0.15));
    thrust->AddLinguisticValue(new LinguisticValue("SlightThrust", 0.1, 0.15, 0.45, 0.5));
    thrust->AddLinguisticValue(new LinguisticValue("HardThrust", 0.45, 0.5, 1, 1));
...
int main()
{
  ...
  FuzzyLogic::FuzzyInput* inputRazdalja = new FuzzyLogic::FuzzyInput("height", 18);
  FuzzyLogic::FuzzyInput* inputPriblizevanje = new FuzzyLogic::FuzzyInput("yvelocity", 4);
  ...
  cout << "the value of outputSignal is " << outputSignal << endl;

  //YOU HAVE TO RESET TOKEN VALUES BEFORE NEW CALCULATIONS!!!
  experiment->ResetTokenValues();

  experiment->BuildMoonLanderThrustRules();
  inputRazdalja = new FuzzyLogic::FuzzyInput("xposition", 0.15);
  inputPriblizevanje = new FuzzyLogic::FuzzyInput("xvelocity", 4);
  h_fuzzyInputs.insert(FuzzyInput_Pairs(inputRazdalja->GetVariableName(), inputRazdalja));
  h_fuzzyInputs.insert(FuzzyInput_Pairs(inputPriblizevanje->GetVariableName(), inputPriblizevanje));
  outputSignal = experiment->CalculateFuzzyRules(h_fuzzyInputs);
  cout << "the value of outputSignal is " << outputSignal << endl;

...
int main()
{
  ...
  while (!CheckWin(Board, turn, 4, move)) 
	...
	int col= bestMove_minmax(Board, 3, -INT_MAX, INT_MAX, turn);
...
int bestMove_minmax(char A[][7], int depth, int alpha, int beta, char player)
{
  if (depth == 0){
  ...
  } else if(CheckWin_board(A, player, 4)){
    if(player==X)
      return -INT_MAX;
    else
      return INT_MAX;
	...
	A[row][i] = player; 
	PrintBoard(A);
	int score = -bestMove_minmax(A, depth-1, -beta, -alpha, opponent(player)); 
...
int CheckVertical(char A[6][7], char player, int cnt, point curr) {
  int count=0;
  int row = curr.row; 
  int col = curr.col; 
  if(row < cnt-1)

...
void Input()
{
  ...
  if (infile.is_open())
   ...
   cout << "I got here A" << endl; 
	  while(ss)
	    {
	      cout << "I got here B" << endl; 

	      string s; 
	      if (!getline(ss, s, ',')) break; 
	      record.push_back(s);
	    }	  
	  state.push_back(record); //注意首行信息也输入进去，即属性  
	}
      infile.close();
    } else {
...
// 计算信息增益，DFS构建决策树  
// current_node 为当前的节点  
// remain_state 为剩余待分类的样例  
// remian_attribute 为剩余还没有考虑的属性  
// 返回根结点指针  
Node * BulidDecisionTreeDFS(Node * p, vector <vector <string> > remain_state, vector <string> remain_attribute){  
